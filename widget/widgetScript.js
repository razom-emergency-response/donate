document.addEventListener("DOMContentLoaded", () => {
    const contentWrapper = document.querySelector(".css-1qsvybm-contentWrapper");
  ...
  `;
    ...
    // Define new Crypto pledge content
      ...
      >
        ...
        <div class="css-okmeyj-adresses">
          <button type="button" class="css-1hzescm-addressCopyButton">
            <span>bc1qrjea0fac9z...arqnmmf6dg27wg</span
    ...
    let btcPrice = 0; // BTC price in USD
    let isFiatInput = false; // false: BTC input, true: USD input
    let isInputFocused = false; // Track input focus
    let selectedAmount = '0.10318753519339872 BTC'; // Default BTC amount (no "BTC")
    ...
    const fetchBtcPrice = async () => {
        try {
            ...
            console.log('Fetched BTC price:', btcPrice);
            updateSubPrice();
        } catch (error) {
    ...
    const updateSubPrice = () => {
        ...
        if (!input || !subPrice || !btcPrice) {
            if (subPrice) subPrice.textContent = '~0.00';
            return;
        }

        // Clean input value by removing non-numeric characters (e.g., $, commas)
        const inputValue = parseFloat(input.value.replace(/[^0-9.]/g, '')) || 0;
        let subPriceValue;

        if (isFiatInput) {
            // Input is USD, sub-price shows BTC equivalent (number only)
            subPriceValue = inputValue / btcPrice; // USD to BTC
            subPrice.textContent = `~${formatCrypto(subPriceValue)}`;
        } else {
            // Input is BTC, sub-price shows USD equivalent (number only)
            subPriceValue = inputValue * btcPrice; // BTC to USD
            subPrice.textContent = `~${formatFiat(subPriceValue)}`;
        }
    };

    // Initialize crypto input when pledge content is loaded
    const initializeCryptoInput = () => {
        ...
        if (!input || !swapButton || !activeCurrency || !calculatedCurrency || !subPrice) {
        ...
        input.addEventListener('input', (event) => {
            ...
            console.log('Input changed:', numericValue, 'isFiatInput:', isFiatInput);
            // selectedAmount = isFiatInput ? `${formatFiat(numericValue)}` : `${formatCrypto(numericValue)} BTC`;
            // Store BTC equivalent
            selectedAmount = isFiatInput ? `${formatCrypto(numericValue / btcPrice)} BTC` : `${formatCrypto(numericValue)} BTC`;
            updateSubPrice();
        });
        ...
        input.addEventListener('focus', () => {
            isInputFocused = true;
            // Clear formatting for easier editing
            const rawValue = parseFloat(input.value.replace(/[^0-9.]/g, '')) || 0;
        ...
        input.addEventListener('blur', () => {
            ...
            input.value = isFiatInput ? formatFiat(value) : formatCrypto(value);
            // selectedAmount = isFiatInput ? `${input.value}` : `${input.value} BTC`;
            selectedAmount = isFiatInput ? `${formatCrypto(value / btcPrice)} BTC` : `${input.value} BTC`;
            console.log('Blur: selectedAmount=', selectedAmount);
            updateSubPrice();
        });

        // Handle currency swap by complex conversion
        /* swapButton.addEventListener('click', () => {
            if (!btcPrice) {
                console.warn('BTC price not available, cannot swap');
                subPrice.textContent = 'Price unavailable';
                return;
            }

            const currentInputValue = parseFloat(input.value.replace(/[^0-9.]/g, '')) || 0;
            console.log('Current input value:', currentInputValue, 'isFiatInput:', isFiatInput);

            isFiatInput = !isFiatInput;
            let newInputValue;

            if (isFiatInput) {
                // Switch to USD input: Convert BTC to USD
                newInputValue = currentInputValue * btcPrice;
                input.value = formatFiat(newInputValue);
                activeCurrency.textContent = 'USD';
                calculatedCurrency.textContent = 'BTC';
            } else {
                // Switch to BTC input: Convert USD to BTC
                newInputValue = currentInputValue / btcPrice;
                input.value = formatCrypto(newInputValue);
                activeCurrency.textContent = 'BTC';
                calculatedCurrency.textContent = 'USD';
            }

            console.log('New input value:', newInputValue, 'isFiatInput:', isFiatInput);
            updateSubPrice();
        }); */

        // Handle currency swap by simply toggling 
        swapButton.addEventListener('click', () => {
          // Store current values
          const inputValue = input.value;
          const subPriceValue = subPrice.textContent.replace(/^~/, ''); // Remove ~ prefix

          console.log('Swapping: input=', inputValue, 'subPrice=', subPriceValue);

          // Swap the values
          input.value = subPriceValue;
          subPrice.textContent = `~${inputValue}`;

          // Toggle currency mode and update labels
          isFiatInput = !isFiatInput;
          if (isFiatInput) {
              activeCurrency.textContent = 'USD';
              calculatedCurrency.textContent = 'BTC';
          ...
          // Update selectedAmount with the new input value (post-swap)
          // selectedAmount = isFiatInput ? `${input.value}` : `${input.value} BTC`;
          const newInputValue = parseFloat(input.value.replace(/[^0-9.]/g, '')) || 0;
          selectedAmount = isFiatInput ? `${formatCrypto(newInputValue / btcPrice)} BTC` : `${formatCrypto(newInputValue)} BTC`;
          console.log('After swap: input=', input.value, 'subPrice=', subPrice.textContent, 'isFiatInput=', isFiatInput, 'selectedAmount=', selectedAmount);
        });

        // Fetch initial price and set up periodic updates
        fetchBtcPrice();
    ...
    document.addEventListener("click", (event) => {
        if (event.target.closest(".css-1hzescm-addressCopyButton") && currentContent === 'donate') {
            ...
            if (addressSpan) {
                ...
                navigator.clipboard.writeText("bc1qrjea0fac9zgjphqk6c0plxf5arqnmmf6dg27wg") // Add your crypto address here
                    .then(() => {
